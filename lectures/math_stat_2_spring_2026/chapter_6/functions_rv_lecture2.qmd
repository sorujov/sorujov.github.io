...
## 📌 Example 1: Simulating Default Times {.smaller}
...
## 📌 Example 2: Log-Normal Distribution {.smaller}
...
## 🎮 Interactive: Log-Normal Parameters {.smaller}
...
## 📌 Example 3: Two-Asset Portfolio Return {.smaller}
...
## 📌 Example 4: Total Insurance Claim Processing Time {.smaller}
...
## 📌 Example 5: Testing Portfolio Variance {.smaller}
...
## 💰 Case Study: Portfolio Return Distribution {.smaller}
...
## 💰 Case Study: Diversification Benefit {.smaller}
...
## 📝 Quiz #1: Transformation Method {.smaller .quiz-question}
...
- [$f_U(u) = f_Y(\ln(u)) \cdot \frac{1}{u}$]{.correct data-explanation="✅ Correct! The transformation formula gives f_U(u) = f_Y(h(u)) * |h'(u)|. Here h(u) = ln(u) and h'(u) = 1/u, so f_U(u) = f_Y(ln(u)) / u. This is exactly how the log-normal distribution arises!"}
- $f_U(u) = f_Y(e^u) \cdot e^u$
- $f_U(u) = f_Y(\ln(u))$
- $f_U(u) = f_Y(\ln(u)) \cdot u$

## 📝 Quiz #2: Stock Price Distribution {.smaller .quiz-question}
...
- [Log-Normal$(\mu, \sigma^2)$]{.correct data-explanation="✅ Correct! Since S/S₀ = eʸ and Y is normal, the ratio follows a log-normal distribution. This is the theoretical foundation of the Black-Scholes model for stock prices."}
- Normal$(\mu, \sigma^2)$
- Exponential$(e^\mu)$
- Gamma$(\mu, \sigma^2)$

## 📝 Quiz #3: Portfolio Return MGF {.smaller .quiz-question}

Two uncorrelated assets have return MGFs $M_1(t)$ and $M_2(t)$. Total portfolio return is $R = R_1 + R_2$. What is $M_R(t)$?

- [$M_1(t) \cdot M_2(t)$]{.correct data-explanation="✅ Correct! For independent random variables, the MGF of the sum equals the product of individual MGFs. This makes analyzing portfolio return distributions remarkably simple when assets are uncorrelated."}
- $M_1(t) + M_2(t)$
- $M_1(t) / M_2(t)$
- $M_1(M_2(t))$

## 📝 Quiz #4: Insurance Claim Aggregation {.smaller .quiz-question}
...
- [Gamma$(5, \beta)$]{.correct data-explanation="✅ Correct! Each exponential MGF is (1-βt)⁻¹. Multiplying 5 gives (1-βt)⁻⁵, the MGF of Gamma(5, β). Actuaries use this to plan staffing for claim processing."}
- Exponential$(5\beta)$
