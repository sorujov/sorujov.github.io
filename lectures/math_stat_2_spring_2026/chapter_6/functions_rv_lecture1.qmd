...
## 📖 Why Study Functions of Random Variables?
...
## 🔍 Example 6.1: Profit from Investment (Discrete) 
...
## 📖 General Approach for Discrete RVs {.larger}

:::::: {.callout-note style="font-size:40px;"}
## 📝 Method for Discrete Transformations
...
## 🔍 Example 6.2: Non-One-to-One Function {.larger}
...
## 📖 Method of Distribution Functions (Continuous) {.smaller}
...
## 🔍 Example 6.3: Linear Transformation
...
::: {.callout-tip}
$U \sim \text{Uniform}(1, 3)$ — Linear transformation of uniform is uniform!
:::
::::
:::::
::::::

## 📊 Interactive: Linear Transformation of Uniform
...

{
  ...
  svg.append("text")
    ...
    .style("font-size", "20px")
    .style("font-weight", "bold")
    .text(`Y ~ Uniform(0,1) → U = ${scale_a.toFixed(1)}Y + ${shift_b.toFixed(1)}`);
  
  return svg.node();
}
```

## 🔍 Example 6.4: Nonlinear Transformation {.larger}
...
viewof trans_type = {
  ...
  const radio = Inputs.radio(
    ["Linear: U = 2Y + 1", "Quadratic: U = Y²", "Cubic: U = Y³", "Square Root: U = √Y"],
...
u_transformed = y_grid.map(y => {
  ...
  if (trans_type === "Square Root: U = √Y") return Math.sqrt(y);
})
...
f_u_values = y_grid.map((y, i) => {
  ...
  if (trans_type === "Square Root: U = √Y") return {u: u, f: 2 * u};
...
## 🧮 Theorem 6.1: General Distribution Function Method {.larger}

::::::: {.callout-important style="font-size:38px;"}
## Theorem 6.1: Distribution Function Technique

Let $Y$ have CDF $F_Y(y)$ and pdf $f_Y(y)$. Let $U = g(Y)$ where $g$ is an **increasing** function with inverse $g^{-1}$.
...
**Intuition:** The Jacobian $\left| \frac{d}{du} g^{-1}(u) \right|$ accounts for how the transformation "stretches" or "compresses" probability.

**Important:** If $g$ is **decreasing**, the same formula works with the absolute value of the derivative.
:::
::::
:::::::

## 💼 Think-Pair-Share: Portfolio Returns {.smaller}
...
## 📊 Case Study: Demonstrating Y² Transformation

::::: columns
:::: {.column width="32%"}
::: {.callout-note style="font-size:32px"}
## 💼 Visualizing the Quadratic Transformation
...
ggplot() +
  ...
  labs(title = "Transformation U = Y² where Y ~ Uniform(0,1)",
       subtitle = "Notice: Density → ∞ as u → 0, matching our theoretical result!",
...
## 📊 Case Study: Real Stock Portfolio Transformation

::::: columns
:::: {.column width="35%"}
::: {.callout-note style="font-size:30px"}
## 💼 Linear Transformation of Stock Returns
...
## 📊 Case Study: Portfolio Statistics
...
## 📝 Quiz #1: Linear Transformations {.quiz-question}

::: {style="font-size:34px"}
A stock return Y follows a Uniform(0, 2) distribution. You transform it to U = 3Y + 1. What is the distribution of U?

- Uniform(0, 6)
- [Uniform(1, 7)]{.correct data-explanation="✅ Correct! Linear transformation of uniform is uniform. Since Y ∈ [0, 2], then U = 3Y + 1 ∈ [1, 7]. The pdf is f_U(u) = 1/6 for 1 ≤ u ≤ 7."}
- Uniform(3, 7)
- Normal(4, 1)
:::

## 📝 Quiz #2: Quadratic Transformation {.quiz-question}
...
- 0.25
- 0.5
- [1.0]{.correct data-explanation="✅ Correct! Using f_U(u) = 1/(2√u) for 0 < u < 1: f_U(0.25) = 1/(2√0.25) = 1/(2×0.5) = 1.0"}
- 2.0
:::

## 📝 Quiz #3: Distribution Function Method {.quiz-question}
...
- [The CDF method properly accounts for the change in probability measure]{.correct data-explanation="✅ Correct! The CDF method correctly handles probability transformations. Direct pdf transformation would miss the Jacobian factor needed to preserve total probability of 1."}
- PDFs cannot be transformed
- It gives the same result either way
:::

## 📝 Quiz #4: Non-One-to-One Functions {.quiz-question}
...
- [Sum probabilities of all y values that map to each u]{.correct data-explanation="✅ Correct! We use p_U(u) = Σ_{y: g(y)=u} p_Y(y) to combine probabilities from all y values that produce the same u value."}
...
-   **Linear transformations** preserve distributional families (e.g., uniform → uniform, normal → normal)
...
✅ Think about when transformations are invertible
