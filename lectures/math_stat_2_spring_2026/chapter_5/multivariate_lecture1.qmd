...
## 📖 Why Study Multivariate Distributions? {.smaller}
...
## 📖 Definition: Joint Probability Function (Discrete) {.smaller}
...
## 🧮 Theorem: Properties of Joint Probability Functions {.smaller}
...
## 📖 Definition: Joint Distribution Function {.smaller}
...
**Geometric View:** $F(y_1, y_2)$ represents the probability accumulated in the "southwest rectangle" from $(-\infty, -\infty)$ to $(y_1, y_2)$.

## 🧮 Theorem: Properties of Joint CDFs {.smaller}
...
## 📖 Definition: Joint Probability Density Function (Continuous) {.smaller}
...
## 🧮 Theorem: Properties of Joint PDFs {.smaller}
...
md`**Density Function:**  
$f(y_1, y_2) = ${c_constant.toFixed(2)} \cdot y_1^{${alpha.toFixed(2)}} \cdot y_2^{${beta.toFixed(2)}}$

**on** $[0,1] \times [0,1]$

**Normalizing constant:**  
$c = (\\alpha+1)(\\beta+1) = ${c_constant.toFixed(2)}$`
```

**Observations:**  

- Higher α increases density near $y_1 = 1$

- Higher β increases density near $y_2 = 1$

- The surface integrates to 1
...
Plot.plot({
  ...
  caption: html`<b>Contour Plot:</b> Joint density f(y₁, y₂) = ${c_constant.toFixed(2)}·y₁<sup>${alpha.toFixed(1)}</sup>·y₂<sup>${beta.toFixed(1)}</sup>`
})
```
...
## 📖 Definition: Marginal Probability Distributions {.smaller}
...
## 📌 Example 5: Finding Marginal Distributions (Discrete) {.smaller}
...
## 📌 Example 6: Finding Marginal Distributions (Continuous) {.smaller}
...
$$f_1(y_1) = \int_0^{y_1} 3y_1 \, dy_2 = 3y_1 \cdot y_1 = 3y_1^2 \quad \text{for } 0 \leq y_1 \leq 1$$

. . .

**Marginal of $Y_2$ (amount sold):**

$$f_2(y_2) = \int_{y_2}^{1} 3y_1 \, dy_1 = \frac{3}{2}[y_1^2]_{y_2}^{1} = \frac{3}{2}(1 - y_2^2) \quad \text{for } 0 \leq y_2 \leq 1$$
...
## 🤝 Think-Pair-Share: Investment Decision {.r-fit-text}

::: {style="font-size: 30px"}
::: {.callout-note}
## 💭 Student Engagement Activity (4 minutes)
...
## 💰 Case Study: Portfolio Returns Analysis (Real Data)
...
## 💰 Case Study: Data Collection {.smaller}
...
cat(sprintf("Min: %.4f%%\n", min(returns_df$AAPL) * 100))
cat(sprintf("Max: %.4f%%\n", max(returns_df$AAPL) * 100))

cat("\n=== MSFT Returns ===\n")
cat(sprintf("Mean: %.4f%%\n", mean(returns_df$MSFT) * 100))
cat(sprintf("Std Dev: %.4f%%\n", sd(returns_df$MSFT) * 100))
cat(sprintf("Min: %.4f%%\n", min(returns_df$MSFT) * 100))
cat(sprintf("Max: %.4f%%\n", max(returns_df$MSFT) * 100))

cat("\n=== Correlation ===\n")
cat(sprintf("Corr(AAPL, MSFT): %.4f\n", 
...
## 💰 Case Study: Joint Distribution Visualization {.smaller}
...
## 💰 Case Study: Marginal Distributions {.smaller}
...
## 📝 Quiz #1: Joint Probability Functions {.smaller .quiz-question}
...
## 📝 Quiz #2: Computing Marginal Distributions {.smaller .quiz-question}
...
## 📝 Quiz #3: Bivariate Density Interpretation {.smaller .quiz-question}

For a continuous bivariate random variable $(Y_1, Y_2)$ with joint pdf $f(y_1, y_2)$, the probability $P(Y_1 \leq a, Y_2 \leq b)$ equals:

- [The volume under the surface $f(y_1, y_2)$ over the region $(-\infty, a] \times (-\infty, b]$]{.correct data-explanation="✅ Correct! For continuous bivariate distributions, probabilities are calculated as volumes under the density surface over the specified region."}
- The value $f(a, b)$
...
- The joint distribution contains **more information** than the marginals alone—it captures the dependence structure between variables
...
✅ Review integration techniques (double integrals)

✅ Think about when two random variables might be independent
