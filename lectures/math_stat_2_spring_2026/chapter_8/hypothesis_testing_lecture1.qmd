...
subtitle: "Fundamentals of Hypothesis Testing"
author:
...
format:
  revealjs:
    ...
    height: 720
    margin: 0.04
    # footer: "Mathematical Statistics – Fundamentals of Hypothesis Testing"
    incremental: false
...
## 📖 Why Hypothesis Testing? {.smaller}
...
::: {.fragment}
3. Ask: *"How likely is this data if $H_0$ were true?"*
:::

::: {.fragment}
4. If the data is **very unlikely under** $H_0$ → **Reject** $H_0$ in favour of $H_a$
:::

::: {.fragment}
5. If the data is **plausible under** $H_0$ → **Fail to reject** $H_0$
...
## 📝 Definition: Null & Alternative Hypotheses {.smaller}
...
## 📝 Definition: Test Statistic & Rejection Region {.smaller}
...
## 📌 Example 1: Voting Probability Test {.smaller}
...
## 📌 Example 1: Result & Interpretation {.smaller}
...
## 📌 Example 2: The $\alpha$-$\beta$ Trade-off {.smaller}
...
## 🧮 Large-Sample Z Test for $\mu$ {.smaller}
...
## 🧮 Large-Sample Z Test for Proportion {.smaller}
...
## 📖 p-Values: Attained Significance {.smaller}
...
## 🔗 Tests ↔ Confidence Intervals {.smaller}
...
## 🎮 Interactive: Visualising Type I & Type II Errors {.smaller}
...
#| eval: false
library(countdown)
countdown(minutes = 4, bottom = 0, right = 0, color_border = "#003366")
```
:::
...
## ✅ Think-Pair-Share: Solution (Continued) {.smaller}
...
The bank's claimed Sharpe ratio of 1.0 is statistically refuted by the 252-day audit data. The strategy is significantly underperforming its advertised risk-adjusted return — a potential misrepresentation to investors.

> 💡 **Key Insight:** A very large $|Z|$ here is driven by the large $n = 252$. Even a modest gap (0.13) becomes highly significant with a full year of daily data.
:::
:::

---

## 💰 Case Study: Testing Zero Mean Return {.smaller}
...
## 📝 Quiz #1: Error Types {.smaller .quiz-question}
...
- [Type I Error — falsely rejecting a true null hypothesis]{.correct data-explanation="✅ Correct! The null hypothesis was true (μ = 2.5%) but the test rejected it. This is a Type I error, with probability α = 0.05."}
...
## 📝 Quiz #2: p-Value Interpretation {.smaller .quiz-question}
...
- [Reject $H_0$; there is statistically significant evidence of a positive mean return]{.correct data-explanation="✅ Correct! p-value = 0.029 < 0.05 = α, so we reject H₀. The evidence supports a positive mean return."}
...
## 📝 Quiz #3: Test Statistic Setup {.smaller .quiz-question}
...
- [$Z = \dfrac{0.008 - 0}{0.030/\sqrt{60}} \approx 2.07$]{.correct data-explanation="✅ Correct! Z = (ȳ - μ₀)/(s/√n) = 0.008 / (0.030/√60) = 0.008 / 0.003873 ≈ 2.07."}
- $Z = 0.008 / 0.030 = 0.267$
...
## 📝 Quiz #4: CI–Test Duality {.smaller .quiz-question}
...
- [Fail to reject $H_0$; since 0 is inside the 95% CI, there is insufficient evidence to conclude $\mu \neq 0$]{.correct data-explanation="✅ Correct! By duality, a two-tailed test at α=0.05 rejects H₀ iff μ₀=0 lies outside the 95% CI. Since 0 ∈ (-0.002, 0.018), we fail to reject."}
