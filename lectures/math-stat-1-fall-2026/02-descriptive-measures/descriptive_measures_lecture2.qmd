...
data2 = {
  ...
  for (let i = 0; i < 2000; i++) {
    ...
    // Scaled to s ~ 1.2, the same spread as the mound-shaped case. Unscaled
    // this lognormal has s ~ 0.37, and on the fixed +/-8% axis it collapsed to
    // a spike -- the one shape whose skew the slide exists to show.
    else if (dist_shape === "skewed right") out.push(2.3 * (Math.exp(0.45 * norm2()) - 1));
...
## ✅ Think-Pair-Share: Which Number?

::: {style="font-size: 30px"}
3. Report the Tchebysheff figure, and say which one you used. Then test the shape assumption: a histogram now, a formal test later.

Daily equity returns are famously heavy-tailed, so the empirical rule understates how often large losses occur — the direction of the error is against you.
...
::: {style="font-size: 28px"}
With real data you would start here — a live download, which we do not run in
class because it changes between lectures:
...
So we use a **simulated sample with the heavy tails equity returns actually
have**, and put the three rules side by side.
:::

---

## 💰 Case Study: The Three Rules Compared

```{r}
#| label: coverage
set.seed(2026)
r <- 0.03 + 0.9 * rt(4000, df = 3)      # heavy-tailed, like daily returns
...
data.frame(
  ...
  actual      = round(sapply(1:3, function(k) mean(abs(r - m) <= k * s)), 4),
  tchebysheff = round(sapply(1:3, function(k) if (k > 1) 1 - 1 / k^2 else 0), 4)
)
```
...
::: {style="font-size: 28px"}
- At $k=1$ the actual coverage **exceeds** 68% — too much mass piled in the middle.

- At $k=3$ it **falls short** of 99.7% — too much mass out in the tails.

- Tchebysheff's bound holds at every $k$, as it must: it assumed nothing about shape.

::: {.fragment}
Both failures at once are the signature of a **heavy-tailed** distribution, and the reason Chapter 4 spends so long on shapes.
:::
:::
